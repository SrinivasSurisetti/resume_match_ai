...
# Static course definitions (previously in courses.py)
...
def get_course_recommendations(reco_field, default_skills=None):
    """
    Returns suggested skills and a list of course names/links based on the predicted field.
    """
    if reco_field == "Data Science":
        recommended_skills = ['Machine Learning', 'Deep Learning', 'TensorFlow', 'Keras']
        course_list = ds_course
    elif reco_field == "Web Development":
        recommended_skills = ['React', 'Django', 'Node.js', 'HTML/CSS']
        course_list = web_course
    elif reco_field == "Mobile Development":
        recommended_skills = ['Kotlin', 'Swift', 'Flutter', 'React Native']
        course_list = android_course
    elif reco_field == "UI/UX Design":
        recommended_skills = ['Figma', 'Adobe XD', 'UX Research', 'Wireframing']
        course_list = uiux_course
    elif reco_field == "Cloud / DevOps":
        recommended_skills = ['AWS', 'Docker', 'Kubernetes', 'CI/CD']
        # DevOps doesn't have a dedicated list currently; default to web
        course_list = web_course
    elif reco_field == "Data Engineering":
        recommended_skills = ['SQL', 'ETL', 'Spark', 'Airflow']
        # Data Eng defaults to DS course list
        course_list = ds_course
    else:
        recommended_skills = default_skills if default_skills else ['Python', 'SQL']
        course_list = web_course

    # Limit to top 5 courses 
    top_courses = course_list[:5]
    return recommended_skills, top_courses

def get_random_support_videos():
    """Returns a random resume tip video and interview tip video."""
    res_vid = random.choice(resume_videos) if resume_videos else None
    int_vid = random.choice(interview_videos) if interview_videos else None
    return res_vid, int_vid
